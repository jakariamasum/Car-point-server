...
async function run() {
  try {
    ...
    app.get("/filteredListings", async (req, res) => {
      ...
      const filteredResult = await productListingsBySellers
        .find(query)
        .sort({ _id: -1 })
        .toArray();
    ...
    app.get("/homeListings", async (req, res) => {
      const result = await productListingsBySellers
        .find()
        .sort({ _id: -1 })
        .toArray();
        ...
        const updateStatus = {
          $set: {
            sellerVerificationStatus:
              updatedVerificationStatus.updatedVerifyStatus,
          },
        };
        const result = await productListingsBySellers.updateMany(
          filter,
          updateStatus
        );
        res.send(result);
    ...
    app.put("/updateListing/:id", verifyToken, async (req, res) => {
      ...
      const result = await productListingsBySellers.updateOne(
        filter,
        updatedDoc,
        options
      );
